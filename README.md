...
- ⛔ **战斗面板/状态页角色名**（2026-06-10 深查后搁置）：ミッシェル/ギンコ/リサ・シルバーマン 等。**数据层面已全部翻好**（SLUS_0_1 strtbl + file1129 名字簇双双写入验证），但游戏运行时不读它们——冷启动+记忆卡+新战斗仍日文，RAM 0x8009F608 的名字数据另有初始化源。需 DuckStation 调试器断点/Ghidra 追。两份译文留存无害
